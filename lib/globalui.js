...
function pluginSrc(sourceEmbedCode) {
  if (location.port || location.domain === 'localhost' || location.domain === '127.0.0.1') {
    return (sourceEmbedCode ? sourceEmbedCode.proto() : "") + "//" + location.hostname + ":" + location.port + location.pathname.replace(/\/$/g, "") + "/" + window.jsFileName;
  } else {
    return (sourceEmbedCode ? sourceEmbedCode.proto() : "") + "//" + window.jsProductionPath.replace(/\/$/g, "") + '/' + window.jsFileName;
  }
}
...
(function($) {
  ...
  $(function() {
    ...
    function resizeGutters() {
      ...
      $("#configure_col").css("min-height", $(window).height() - $("#body").offset().top);
    }
