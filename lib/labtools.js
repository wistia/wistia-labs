...
(function(W, $) {
  ...
  W.EmbedCode = (function() {
    ...
    EmbedCode.prototype.getOption = function(key) {
      return W.obj.get(this.options(), key);
    };
    ...
    EmbedCode.prototype._castOptions = function() {
      return W.obj.castDeep(this._options);
    };
  ...
  W.EmbedCode.fromOembed = function(hashedId, options, callback) {
    ...
    return $.getJSON(window.location.protocol + "//" + (W.Hosts.cdnFastWistiaComHost()) + "/oembed.json?callback=?", {
  ...
  W.EmbedCode.oembedUrl = function(hashedId, options) {
    ...
    return (options.ssl ? "https:" : "http:") + "//" + (W.Hosts.cdnFastWistiaComHost()) + "/embed/medias/" + hashedId + "?" + (W.url.jsonToParams(options));
  };
  ...
  W.EmbedCode.serializeElem = function(elem) {
    var attr, child, contents, i, isValidName, isValidVal, j, k, len, name, pairs, ref, ref1, tag, val;
    if (elem.nodeType === 1) {
      pairs = [];
      for (i = j = 0, ref = elem.attributes.length; 0 <= ref ? j < ref : j > ref; i = 0 <= ref ? ++j : --j) {
        attr = elem.attributes.item(i);
        name = attr.nodeName.toLowerCase();
        val = attr.nodeValue;
        isValidName = W.EmbedCode._attrWhitelist[name] || /^data-/.test(name);
        isValidVal = (val != null) && val !== "";
        if (isValidName) {
          if ((val != null) && val !== '') {
            pairs.push(name + "=\"" + val + "\"");
          } else {
      ...
      if (/^(br|hr|img|link|meta|input)$/i.test(tag)) {
        ...
        return "<" + tag + " " + (pairs.join(" ")) + ">" + contents + "</" + tag + ">";
      }
    } else {
  ...
  W.EmbedCode.parse = function(embedCode) {
    if (W.EmbedCode.isIframe(embedCode)) {
      ...
      return W.AsyncEmbedCode.parse(embedCode);
  ...
  W.EmbedCode.isAsync = function(embedCode) {
    var e, error;
    if (embedCode instanceof W.AsyncEmbedCode && embedCode.isValid()) {
      return true;
    }
    try {
      return new W.AsyncEmbedCode(embedCode).isValid();
    } catch (error) {
      e = error;
      return false;
    }
  ...
  W.EmbedCode.isValid = function(embedCode) {
    var ref;
    return !!((ref = W.EmbedCode.parse(embedCode)) != null ? ref.isValid() : void 0);
  };
  W.PopoverEmbedCode = (function(superClass) {
    extend(PopoverEmbedCode, superClass);
    ...
    PopoverEmbedCode.prototype.parse = function(embedCode) {
      var j, key, len, match, matches, ref, ref1, val;
      ...
      if (matches = this._$popover.attr('class').match(/wistia-popover(?:\[([^\]]+)\])?/)) {
        ...
        for (j = 0, len = ref.length; j < len; j++) {
          match = ref[j];
          ref1 = match.split('='), key = ref1[0], val = ref1[1];
    ...
    PopoverEmbedCode.prototype.isValid = function() {
      var ref;
      return ((ref = this._$popover) != null ? ref.length : void 0) && this._width && this._height && this._options;
    };
    ...
    PopoverEmbedCode.prototype._classConfig = function() {
      var key, val;
      return "wistia-popover[" + ((function() {
        var ref, results;
        ref = this._config;
        results = [];
        for (key in ref) {
    ...
    PopoverEmbedCode.prototype.toString = function() {
      var result;
      result = (W.EmbedCode.serializeElem(this._$popover[0])) + "\n" + (this._scripts.join("\n"));
  ...
  W.IframeEmbedCode = (function(superClass) {
    extend(IframeEmbedCode, superClass);
    ...
    IframeEmbedCode.prototype.isValid = function() {
      var ref;
      return ((ref = this._$iframe) != null ? ref.length : void 0) && this._width && this._height && this._options;
    };
    ...
    IframeEmbedCode.prototype.toString = function() {
      var result;
      result = W.EmbedCode.serializeElem(this._$iframe[0]);
      return result.replace(/&amp;/g, "&");
    };
  ...
  W.ApiEmbedCode = (function(superClass) {
    extend(ApiEmbedCode, superClass);
    ...
    ApiEmbedCode.prototype.parse = function(embedCode) {
      ...
      this._options = W.ApiEmbedCode.parseOptions(this._rawOptions) || null;
    ...
    ApiEmbedCode.prototype.options = function(newOptions) {
      var hashedIdAndOptions, matches, newRawOptions;
      if (newOptions != null) {
        ...
        if (this._rawOptions) {
          ...
          matches = this._embedCode.match(W.ApiEmbedCode.rhashedid);
          hashedIdAndOptions = "" + (matches[0].replace(this._hashedId + "\"", this._hashedId + "\", " + newRawOptions));
          this.parse(this._embedCode.replace(matches[0], hashedIdAndOptions));
          this._rawEmbedCode.replace("\"" + "\"");
        }
        return this;
      } else {
        return W.extend({}, this._options);
    ...
    ApiEmbedCode.prototype.ssl = function() {
      var j, len, ref, scriptTag;
      ref = W.util.scriptTags(this._rawEmbedCode);
      for (j = 0, len = ref.length; j < len; j++) {
        scriptTag = ref[j];
    ...
    ApiEmbedCode.prototype.isValid = function() {
      return this._containerId && this._handle && this._hashedId && this.height() && this.width();
    };
    ...
    ApiEmbedCode.prototype.toString = function() {
      var result;
      result = (W.EmbedCode.serializeElem(this._containerElem[0])) + "\n" + (this._scripts.join("\n"));
      return result.replace(/&amp;/g, "&");
    };
  ...
  W.ApiEmbedCode.evilDeepCopy = function(h, options) {
  ...
  W.AsyncEmbedCode = (function(superClass) {
    extend(AsyncEmbedCode, superClass);

    function AsyncEmbedCode(embedCode) {
      if (embedCode) {
        this.parse(embedCode);
      }
    }

    AsyncEmbedCode.prototype.parse = function(embedCode) {
      this._embedCodeFrag = $("<div>");
      this._rawEmbedCode = embedCode;
      this._rawHtml = W.util.removeScriptTags(embedCode).replace(/\n+$/g, "\n");
      this._embedCodeFrag.html(this._rawHtml);
      ...
      this._containerContents = this._containerElem.html();
      this._containerHtml = $("<div>").html(this._containerElem.clone()).html();
      this._hashedId = W.AsyncEmbedCode.hashedId(this._rawEmbedCode);
      this._options = W.embeds.optionsFromElemClass(this._containerElem);
      this._html = W.util.removeScriptTags(this._rawEmbedCode);
      this._scripts = W.util.scriptTags(this._rawEmbedCode);
      this._embedCode = this._scripts.join("\n") + "\n" + this._embedCodeFrag.html().replace(/\s+$/g, "");
      return this;
    };

    AsyncEmbedCode.prototype.options = function(newOptions) {
      if (newOptions != null) {
        this._options = newOptions;
        this._containerElem.removeAttr('class');
        this._containerElem.addClass('wistia_embed');
        this._containerElem.addClass("wistia_async_" + this._hashedId);
        if (this.optionsAsClasses().length > 400) {
          this._optionsScript = this.optionsAsJavascript();
        } else {
          this._optionsScript = null;
          this._containerElem.addClass(this.optionsAsClasses());
        }
        return this;
      } else {
        return W.obj.clone(this._options);
      }
    };

    AsyncEmbedCode.prototype.optionsAsClasses = function() {
      var optionPairs;
      optionPairs = [];
      W.obj.eachLeaf(this._options, (function(_this) {
        return function(leafVal, path) {
          return optionPairs.push(encodeURIComponent(W.url._brack(path)) + "=" + encodeURIComponent(leafVal));
        };
      })(this));
      return optionPairs.join(' ');
    };

    AsyncEmbedCode.prototype.optionsAsJavascript = function() {
      return "<script>\nwindow._wq = window._wq || [];\nwindow._wq.push({ \"" + this._hashedId + "\": " + (W.ApiEmbedCode.evilJsonStringify(this._options)) + "})\n</script>";
    };

    AsyncEmbedCode.prototype.containerContents = function(newContents) {
      if (newContents != null) {
        this._containerElem.html(newContents);
        return this;
      } else {
    ...
    AsyncEmbedCode.prototype.containerHtml = function() {
      return this._containerHtml;
    };

    AsyncEmbedCode.prototype.hashedId = function(newHashedId) {
      if (newHashedId != null) {
        this._hashedId = newHashedId;
        return this;
      } else {
        return this._hashedId;
      }
    ...
    AsyncEmbedCode.prototype.css = function(prop, val) {
      if (val != null) {
        this._containerElem.css(prop, val);
        return this;
      } else {
    ...
    AsyncEmbedCode.prototype.width = function(w) {
      if (w != null) {
        this.css('width', w);
        return this;
      } else {
    ...
    AsyncEmbedCode.prototype.height = function(h) {
      if (h != null) {
        this.css('height', h);
        return this;
      } else {
    ...
    AsyncEmbedCode.prototype.isValid = function() {
      return this._hashedId && this.height() && this.width();
    };

    AsyncEmbedCode.prototype.fromOembed = function(options, callback) {
      options = W.extend({
        height: this.height(),
        width: this.width()
      }, this.options(), options);
    ...
    AsyncEmbedCode.prototype.toString = function() {
      var result, scripts;
      scripts = W.obj.clone(this._scripts);
      if (this._optionsScript) {
        scripts.unshift(this._optionsScript);
      }
      result = (scripts.join("\n")) + "\n" + (W.EmbedCode.serializeElem(this._containerElem[0]));
      result = result.replace(/&amp;/g, "&");
      return result;
    };

    return AsyncEmbedCode;

  })(W.EmbedCode);
  W.AsyncEmbedCode.parse = function(embedCode) {
    return new W.AsyncEmbedCode(embedCode);
  };
  W.AsyncEmbedCode.roptions = /Wistia\.embed\(.+?,\s*([\s\S]+)\s*\)/;
  W.AsyncEmbedCode.rhashedid = /wistia_async_([^ "]+)/;
  return W.AsyncEmbedCode.hashedId = function(embedCode) {
    var matches;
